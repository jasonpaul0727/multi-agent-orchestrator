...
2026-09-24 live validation: `test_systemd_scope_contains_preexec_user_mount_overlay_and_cgroup_limits` now creates the OverlayFS candidate inside the real systemd scope, returns only the candidate manifest to the host-side test, then acquires the fenced workspace lease and publishes through the journaled publisher. It verifies the workspace changed and the lower tree did not. This supersedes the earlier statement above that the live Systemd/OverlayFS-to-host candidate publication path had not been tested; it still does not exercise an untrusted Worker or Approval/Gateway/Secret Broker integration.

...
任务生命周期与失败恢复设计已逐节获得用户确认并完成自审。V1 采用事件驱动 DAG，并允许执行期间仅追加和细化节点；优先保证可控可靠。正式规格位于 `docs/superpowers/specs/2026-09-09-task-lifecycle-design.md`，当前等待用户审阅书面规格。
...
1. 用户审阅并批准任务生命周期与失败恢复书面规格。
2. 设计三种预设和 DIY 配置格式及路由规则。
3. 设计权限、安全、隔离和审批模型。
4. 设计持久化、成本统计、可观测性和测试方案。
5. 汇总并审阅完整产品设计文档。
6. 用户批准完整设计文档后，编写实施计划，再开始编码。

...
2026-09-23 P0/P4 隔离更新：WSL2 的 systemd transient service/cgroup 实际资源限制与超限终止、进程树 kill、user/mount/network namespace、PrivateTmp/ProtectHome、workspace bind、`.git`/控制目录只读、Git worktree metadata 隐藏、Landlock 越界拒绝和 4 MiB tmpfs Overlay 写满/lower 不变已实测。现有 `SystemdReadOnlyLauncher` 已把 cgroup/rlimit 校验、descriptor-anchored no-follow 工作区快照、只读 runtime/workspace bind、私有网络/tmp/Home、敏感控制目录隐藏、Landlock 和有界输出/超时/取消接成真实启动路径；另有 5 个 live integration tests 覆盖启动器读写/网络/秘密边界和进程终止。全量测试及 90.24% 覆盖率门槛通过。该组件仍是内部只读后端切片：没有 race-safe workspace-write、Overlay diff 导出/原子应用、Tool Gateway/Secret Broker/Approval 或 Worker 接线；平台矩阵见 `docs/security/platform-support.md`，WSL2 不是完整 V1 支持平台，V1 仍不可交付。
...
- 权限、安全规格已于 2026-09-22 获用户书面批准。2026-09-23 的 `SystemdReadOnlyLauncher` 已将 WSL2 live probes 接成真实、fail-closed 的只读命令启动器；workspace-write、差异收集、Tool Gateway/Worker 接入仍未完成，当前 WSL2 仅有该内部只读 profile 的验证证据，不可宣称完整 V1 平台已支持。
